...
def load_experiment_results(result_dir, experiments):
    ...
    for key, experiment in experiments.items():
        ...
        # eigenvalues
        eigenvalues = np.genfromtxt(os.path.join(file_dir, "eigenvalues.csv"), delimiter=',', skip_header=1).reshape(-1, 2, N_TRAJECTORIES).transpose([2, 0, 1])
        # state trajectories
        data = np.empty((N_T, N_TRAJECTORIES * N_STATES + 1, 0))
        for i in range(N_STATES):
            data = np.concatenate([data, np.genfromtxt(os.path.join(file_dir, f"state_{i}_trajectories.csv"), delimiter=',', skip_header=1)[:,:,np.newaxis]], axis=-1)
        ...
        if key == "reference":
            ...
            data = np.genfromtxt(os.path.join(file_dir, "rms_error_state_dom0.csv"), delimiter=',', skip_header=1).transpose([1, 0])
            t_error = data[0]
        state_error = data[1:N_TRAJECTORIES + 1]
        results[key] = dict(t=t, x=x, t_error=t_error, state_error=state_error, eigenvalues=eigenvalues)
    return results
...
def main():
    ...
    experiments = {
        "reference": Experiment(file_path="reference", linestyle="solid", color="black", marker="o"),
        "lti": Experiment(file_path="lti", linestyle="solid", color="cyan", marker="v"),
        "phin": Experiment(file_path="phin", linestyle="solid", color="orange", marker="d"),
        "mi": Experiment(file_path="mi", linestyle="solid", color="magenta", marker="s"),
    }
