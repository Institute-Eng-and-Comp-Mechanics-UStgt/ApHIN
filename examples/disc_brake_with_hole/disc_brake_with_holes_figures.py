...
def load_experiment_results(result_dir, experiments):
    ...
    for key, experiment in experiments.items():
        ...
        # rms error
        data = np.genfromtxt(
            os.path.join(file_dir, "rms_error_state_disp.csv"),
            delimiter=",",
            skip_header=1,
        ).transpose()
        t = data[0]
        rms_disp = data[1 : N_TRAJECTORIES + 1]
        rms_temp = np.genfromtxt(
            os.path.join(file_dir, "rms_error_state_temp.csv"),
            delimiter=",",
            skip_header=1,
        ).transpose()[1 : N_TRAJECTORIES + 1]
        rms_vel = np.genfromtxt(
            os.path.join(file_dir, "rms_error_state_vel.csv"),
            delimiter=",",
            skip_header=1,
        ).transpose()[1 : N_TRAJECTORIES + 1]
        ...
        if os.path.exists(state_traj_file):
            data = np.genfromtxt(
                os.path.join(file_dir, "db_custom_nodes.csv"),
                delimiter=",",
                skip_header=1,
            )
...
def plot_trajectories(results):
    ...
    for i, (quantity, c) in enumerate(zip(["temp", "disp", "vel"], colors)):
        for j in range(N_STATES):
            ...
            axs[i, 0].plot(
                ...
                linestyle="solid",
                color=c,
                label="Experiment r12",
            )
        # for j in range(N_TRAJECTORIES):
        axs[i, 1].semilogy(
            data["t"], data[f"rms_{quantity}"].mean(axis=0), color=c, zorder=10
        )
        axs[i, 1].semilogy(data["t"], data[f"rms_{quantity}"].transpose(), color="gray")
    ...
    for k, (name, data) in enumerate(zip(series_names, series_data)):
        ...
        ax.boxplot(
            ...
            boxprops=dict(
                linewidth=0.8,
                facecolor=to_rgba(colors[k], alpha=0.15),
                edgecolor=colors[k],
            ),
